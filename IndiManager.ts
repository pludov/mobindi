...
export default class IndiManager implements RequestHandler.APIAppProvider<BackOfficeAPI.IndiAPI>{
    ...
    addToProfile= (ct: CancellationToken, payload: { uid: string; dev: string; vec: string; prop: string|null }) => {
        return this.profileManager.addToProfile(ct, payload);
    }

    removeFromProfile= (ct: CancellationToken, payload: { uid: string; dev: string; vec: string; prop: string|null }) => {
